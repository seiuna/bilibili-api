...
`ensureLogin(options?)` attempts cookie validation, then refresh when applicable, then QR only for a known logged-out state. A refresh rejection falls back to QR only for explicit `code: -101`; risk-control/unknown rejection codes are not assumed to mean expired credentials. Explicit refresh `-101` rejection during the initial nav probe also permits QR fallback without repeating the rejected refresh. A successful refresh whose nav retry still explicitly returns `-101` also permits QR fallback without another refresh attempt. This applies to the initial nav probe and the later login recheck. Other refresh failures and HTTP/network/JSON, malformed-payload, or non-auth business errors propagate. QR fallback is attempted at most once per `ensureLogin()` call; its own failure propagates rather than triggering another QR attempt. QR/TV/password/logout calls use the configured `customFetch` transport. Failed logout business responses throw before returning an unauthenticated typed client.
